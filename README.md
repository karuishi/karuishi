<h1 align="center">Hi there! 👋 I'm Pedro Lucas</h1>
<h3 align="center">Passionate about Assistive Technology, Robotics, and Artificial Intelligence 🤖</h3>

<p align="center">
  My ultimate dream is to become a University Professor while contributing to innovations in assistive navigation systems and service robots. My core goal is to develop cutting-edge assistive technologies that enhance the independence and quality of life for individuals with disabilities.
</p>

<p align="center">
  <a href="mailto:pedrolucas.js18@gmail.com">
    <img alt="Email" src="https://img.shields.io/badge/Email-pedrolucas.js18%40gmail.com-blue?style=flat-square&logo=gmail">
  </a>
  <a href="https://linkedin.com/in/YOUR-LINKEDIN-URL">
    <img alt="LinkedIn" src="https://img.shields.io/badge/LinkedIn-Connect-blue?style=flat-square&logo=linkedin">
  </a>
</p>

---

### 👨‍🎓 Background & Research
- 🎓 **Bachelor of Information Systems** (Undergraduate) at **UNEB**
- 🔬 Researcher in the **PICIN/UNEB** scientific initiation scholarship program.
- 🏆 Active participant in national robotics competitions across Brazil.

### 🚀 Current Projects & Focus
- ♿ **BahiaRT:** Developing an assistive navigation system for smart wheelchairs.
- 🧭 **ROS 2 & Nav2:** Focused on testing, documenting, and enhancing autonomous navigation capabilities and robotic skills.
- 🐳 Utilizing **Docker** containerization systems to manage development dependencies for robotic navigation workspaces.

### 💻 Tech Stack

**Languages & Core** <p>
  <img alt="C++" src="https://img.shields.io/badge/-C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white" />
  <img alt="Python" src="https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img alt="Java" src="https://img.shields.io/badge/-Java-007396?style=flat-square&logo=java&logoColor=white" />
</p>

**Robotics & Environments** <p>
  <img alt="ROS 2" src="https://img.shields.io/badge/-ROS_2-22314E?style=flat-square&logo=ros&logoColor=white" />
  <img alt="Docker" src="https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white" />
  <img alt="Ubuntu" src="https://img.shields.io/badge/-Ubuntu-E95420?style=flat-square&logo=ubuntu&logoColor=white" />
</p>

**Web & Tools** <p>
  <img alt="HTML5" src="https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" />
  <img alt="CSS3" src="https://img.shields.io/badge/-CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" />
  <img alt="JavaScript" src="https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
  <img alt="Flask" src="https://img.shields.io/badge/-Flask-000000?style=flat-square&logo=flask&logoColor=white" />
  <img alt="Git" src="https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white" />
</p>

### 📚 What I'm Learning
- Advanced implementations of **Nav2** and **ROS 2**.
- Diving deeper into Robotic Hardware for more efficient and practical designs.
- Fundamentals and practical applications of **Data Science**.

---

### 📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=karuishi&show_icons=true&theme=radical" alt="Pedro's GitHub Stats" />
</p>

<p align="center">
  <i>Let's connect for collaboration or to chat about robotics and assistive tech!</i>
</p>
